"use strict";
var Gauge = (function () {
    function Gauge(gaugeCanvas, gaugeSettings) {
        this.gaugeCanvas = gaugeCanvas;
        this.index = 0; // lookup table index associated with the current input level of gauge
        ...
        this.endDegree = 405;
        this.outerScaleDiam = 100;
        this.outerScaleRange = { s: 0, e: 6000 };
        this.outerScaleIntervals = [500, 100, 0];
        this.outerScaleFnt = "8pt Calibri";
        this.outerScaleColor = "#000000";
        this.outerScaleUnit = "PSI";
        this.hasInnerScale = false;
        this.innerScaleConvFactor = 0.068573;
        this.innerScaleDiam = 96;
        this.innerScaleMin = 0;
        this.innerScaleFnt = "8pt Arial";
        this.innerScaleColor = "#ff0000";
        this.innerScaleUnit = "Kg/cm3";
        this.innerScaleToEnd = true;
        this.dialBaseDiam = 25;
        this.pointerColor = "#0000ff";
        this.digWd = 60;
        this.digHt = 14;
        this.digFnt = "12pt Calibri";
        this.digBC = "#efefef";
    ...
    Gauge.prototype.drawScreen = function () {
        var digDispX = this.centerX - (this.digWd / 2);
        var digDispY = this.centerY + this.outerScaleDiam + (this.digHt / 2);
        var maxDigits = Math.floor(this.outerScaleRange.e).toString().length;
        var digDispFixed = 0;
        if (this.inputGranularity < 1) {
            ...
            if (decs.substr(decs.length - 1, 1) == "0") {
                addTo++;
                if (decs.substr(decs.length - 2, 1) == "0")
                    addTo++;
            }
            maxDigits += addTo;
        ...
        var gaugeNmFS = parseInt(this.gaugeNmFnt.replace(/[^0-9 ]/g, ""));
        if (!gaugeNmFS)
            gaugeNmFS = 18;
        var outerScaleFS = parseInt(this.outerScaleFnt.replace(/[^0-9 ]/g, ""));
        if (outerScaleFS)
            outerScaleFS = 8;
        var innerScaleFS = parseInt(this.innerScaleFnt.replace(/[^0-9 ]/g, ""));
        if (!innerScaleFS)
            innerScaleFS = 8;
        var digFS = parseInt(this.digFnt.replace(/[^0-9 ]/g, ""));
        if (!digFS)
            digFS = 12;
        //***** unit label positions *****//
        var gap = ((this.centerY + this.outerScaleDiam) - (this.centerY + this.dialBaseDiam) - ((outerScaleFS + innerScaleFS) * 1.25)) / 7;
        var minorUnitY = this.centerY + this.dialBaseDiam + (gap * 3);
        var majorUnitY = minorUnitY + innerScaleFS + gap;
        // current input level
        ...
        if (this.hasInnerScale) {
            var innerStart = this.outerScaleRange.s * this.innerScaleConvFactor, innerEnd = this.outerScaleRange.e * this.innerScaleConvFactor, innerTotUnits = (innerEnd - innerStart);
            // inner scale arc
    ...
    Gauge.prototype.setInputLevel = function (input) {
        ...
        if (this.index < 0)
            this.index = 0;
        if (this.index >= this.maxSteps)
            this.index = this.maxSteps - 1;
    }; // end setInputLevel()
    return Gauge;
}());
exports.Gauge = Gauge; // end class
function createLookupTbl(startRad, endRad, steps) {
    // assume all parameters are positive -- validation to be added
    var table = [], ang = startRad, angStep = (endRad - startRad) / steps;
...
function drawOuterSteps(context, table, d1, d2, cX, cY, color, w, interval, granularity, label, lblFnt, startRad, endRad, firstLabel) {
    if (typeof label === "undefined")
        label = false;
    var last, skipSteps = interval / granularity, totRadians = endRad - startRad;
    context.strokeStyle = color;
    context.lineWidth = w;
    for (var i = 0; i < table.length; i = i + skipSteps) {
        drawTickMark(context, table[i][0], table[i][1], d1, d2, cX, cY);
        if (label) {
            var lbl = ((i * granularity) + firstLabel).toString(), angle = startRad + (totRadians * (i / table.length));
...
function drawInnerSteps(context, table, d1, d2, cX, cY, color, w, interval, firstLabel, innerStart, innerEnd, toEnd, label, lblFnt, startRad, endRad) {
    ...
    context.strokeStyle = color;
    context.lineWidth = w;
    var totUnits = innerEnd - innerStart, last = innerEnd, idx = 0, totRadians = endRad - startRad;
